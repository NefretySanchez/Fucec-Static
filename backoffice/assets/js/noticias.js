...
var messagesUIManager = {
  ...
  drawItem: function (itemData) {
    ...
    let phone = itemData.foto;
    let subject = itemData.descripcion_corta;
    let datetime = itemData.contenido;
  ...
  updateNews: function () {
    if (
      ...
      document.getElementById("categoria").value.trim().length === 0
    ) {
  ...
  addNews: function () {
    if (
      ...
      document.getElementById("createCategoria").value.trim().length === 0
    ) {
      alert("Debes completar los campos para continuar");
      return false;
    }
